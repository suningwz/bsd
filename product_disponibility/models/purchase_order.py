...
class PurchaseOrder(models.Model):
    ...
    reception_date = fields.Date(string=_("Date de Réception Estimée"), default=lambda self: self._get_default_r_date(), track_visibility="onchange")
    date_disponibility = fields.Date(string=_("Date de Mise à Disposition Estimée"),default=lambda self: self._get_default_d_date(), track_visibility="onchange")
    sales_count = fields.Float(compute="_compute_sales_count", string="Vendu(s)")
    uom_name = fields.Char(related="product_id.uom_name", string="UOM")
    ...
    def _onchange_date_order(self):
        self.date_disponibility = str(self.date_order + datetime.timedelta(weeks=16) + datetime.timedelta(days=8))
        self.reception_date = str(self.date_order + datetime.timedelta(weeks=16))

    # TODO : make this 8 days a parameter
    ...
    def action_view_sales_po(self):
        ...
        sale_order_line_ids = SaleOrderLine.search([('product_id', 'in', list(product_ids))])

        for line in sale_order_line_ids:
            sale_lines.append(line.id)

        sale_order_ids = SaleOrder.search([('order_line', 'in', sale_lines)])
...
class PurchaseOrderLine(models.Model):
    ...
    date_disponibility = fields.Date(string=_("Date de MàD"))
